...
Now, read training data including training set, training labels, and training subjects:
...
Now, we merge training and test data:
```{r}
merged_data <- rbind(train_set2, test_set2)
```

Next step is to rename variables and use meaningful names:
```{r}
names(merged_data) <- c("subject",  as.character(variables$V2), "activity_label")
```

According to the project description, we only need **mean** and **std** of the measurements. Therefore, we filter out mean and std-based variables :
...
The following script adds descriptive activity names to the data set:
```{r}
...
The final step is to analyze the data and report a group-wise summary.
